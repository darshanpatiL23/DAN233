...
fig2 = px.bar(df["Preferred_Product"].value_counts(), title="Product Preference")
st.plotly_chart(fig2)

# ----- Preprocessing -----
df_ml = df.copy()
le = LabelEncoder()
df_ml["Income"] = le.fit_transform(df_ml["Income"])
df_ml["Preferred_Product"] = le.fit_transform(df_ml["Preferred_Product"])
...
st.write("Regression Model trained for spend prediction (demo)")
...
income_enc = le.transform([income])[0]
...
st.write("Purchase Probability:",pred)
